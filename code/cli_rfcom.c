...
/*
void fun(unsigned char * put,char *name,unsigned short int len)
{
    int num=0;
    char   buf[64];
    ...
    put[0]=0x02;
    put[2]=(unsigened char)len;
    put[1]=(unsigened char)(len>>8);
    put[3]=0xcd;
    put[7]=0x01;
...
int main(int argc,char *argv[])
{
    ...
    while(1)
    {
	    ...
	    printf("send\n");                         //start put
	    status=write(sock,PUT,sizeof(PUT));
	    if(status<0)
	    {
	        perror("send");
		return -1;
	    ...
	    printf("\n");
            ...
            printf("end\n");
	    status=write(sock,put,sizeof(put));      //end
	    if(status<0)
	    {
	        perror("write");
		return -1;
