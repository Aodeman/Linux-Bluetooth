...
/*用指定的的dongle 去搜索周围的Bluetooth device。并将搜索到的Bluetooth device bdaddr传递回来
 *dev_id ：指定的dongle
 *len    ：此次搜索的时间长度（1=1.25s）
 *nrsp   ：搜索的最多remote device 。小于0则取255.
 *lap    : bdaddr 的LAP部分，通常使用NULL，自动设置
 *ii     : 存放搜索到的Bluetooth device 的地方。一个存放inquiry_info指针的地址，自动分配空间。
 *flags  ：搜索flags ,使用IREQ_CACHE_FLUSH，在搜索到一个device后会重新inquiry，否则可能会返回上次的结果。
 *return ：搜索到device 的结果。
 * */
